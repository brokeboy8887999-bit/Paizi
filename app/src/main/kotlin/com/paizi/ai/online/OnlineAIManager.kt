...
class OnlineAIManager(
    ...
    suspend fun executeRequest(
        ...
        val isGemini = provider.baseUrl.contains("generativelanguage.googleapis.com") || provider.name.contains("Gemini", ignoreCase = true)

        try {
            if (isGemini) {
                executeGeminiRequest(client, provider, effectiveApiKey, systemPrompt, userPrompt)
            } else {
                executeOpenAiCompatibleRequest(client, provider, effectiveApiKey, systemPrompt, userPrompt)
            }
    ...
    private fun resolveApiKey(provider: OnlineProviderConfigEntity): String {
    ...
    private fun executeGeminiRequest(
        ...
        if (apiKey.isBlank()) {
            throw OnlineAiException(ErrorType.AUTHENTICATION, "API key is required for ${provider.name}")
        }

        val url = "${provider.baseUrl.trimEnd('/')}/models/${provider.modelName}:generateContent?key=$apiKey"
